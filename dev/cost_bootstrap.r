...
bootstrapping <- function(dat, n = 100) {
    ...
    map_dbl(sample_list, \(sample) {
        with(dat[sample, ], mean(price)) #sum(miles) / sum(gallons))
    })
}
...
generate_bootstraps <- function(dat, var, custom_var) {
    ...
    bootstraps %>%
        ...
        labs(
            x = "Bootstrapped fuel cost distribution (in $/gallon)",
            y = NULL,
            title = glue::glue("Estimated fuel cost distribution by {custom_var}")
        ) +
        theme(
...
ggsave(
    filename = "graphs/cost_average_bootstrap.png",
