...
fitdata <- altima %>%
    mutate(
        ...
        mpg_cat = cut(mpg, breaks = c(0, seq(16, 22, 2), 100))
    )
...
p <- (cut_mpg_g +full_set_g) / (max_miles_g + plot_spacer())  +
    plot_annotation(
        title = "When should I fill up my 2011 Nissan Quest?",
        subtitle = "When the miles counter reaches this number, you'll be fine in 95% of cases."
    )

ggsave("dev/when_to_fill_up_quest.png",
        width = 10, height = 8,
        plot = p)